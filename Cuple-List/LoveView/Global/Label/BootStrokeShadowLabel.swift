...
class BootStrokeShadowLabel: UIView {//边框
    
    // ✅ MARK: 新增：字符间距属性（核心解决报错）
    var letterSpacing: CGFloat = 0.0 {
        didSet {
            if oldValue != letterSpacing {
                updateLabel() // 间距变化时刷新文本
            }
        }
    }
    ...
    /// 阴影偏移量
    var shadowOffset: CGSize = CGSize(width: 0, height: 1) { // 默认值改为(0,1)
        didSet { updateLabel() }
    }
    ...
    // ✅ MARK: 核心修改：重构updateLabel方法，集成「字符间距+原有所有样式」
    private func updateLabel() {
        // 1. 更新阴影属性（保留原有逻辑）
        label.layer.shadowColor = shadowColor.cgColor
        label.layer.shadowOffset = shadowOffset
        label.layer.shadowRadius = shadowBlurRadius
        label.layer.shadowOpacity = 1.0
        
        // 2. 构建富文本属性字典（集成 字符间距+字体+颜色+描边 所有属性）
        ...
        // ✅ 核心新增：添加字符间距属性，支持 letterSpacing 配置
        if letterSpacing > 0 {
            attributes[.kern] = letterSpacing
        }
        
        // 3. 生成带所有样式的富文本，赋值给子Label
...
class BootBorderGradienLabel: UILabel {
