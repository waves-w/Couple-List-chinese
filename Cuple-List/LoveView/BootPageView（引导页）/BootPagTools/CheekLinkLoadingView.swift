...
class CheekLinkLoadingView: UIViewController {
    
    private let partnerCode: String
    /// dismiss 加载页后由 CheekBootPageView push CheekLinkSuccessView
    var onModalLinkSuccess: (() -> Void)?
    ...
    private func setupUI() {
        ...
        statusLabel = UILabel()
        statusLabel.text = "Waiting to connect to the other device"
...
//            make.height.lessThanOrEqualTo(200)
        ...
        failedTitleLabel.text = "Link failed"
    ...
    private func dismissModalAndShowLinkSuccess() {
        UnlinkConfirmPopup.forceRemoveFromAllWindows()
        let callback = onModalLinkSuccess
        dismiss(animated: true) {
            callback?()
        }
    }
