...
class BirthdayDatePopup: NSObject {
    ...
    private func setupUI() {
        ...
        continueButton.reactive.controlEvents(.touchUpInside).observeValues {
            ...
            print("✅ BirthdayDatePopup: 选中日期：\(selectedDate)")
            self.popup.dismiss(animated: true)
        }
    ...
    func show(width: CGFloat, bottomSpacing: CGFloat, initialDate: Date? = nil) {
        ...
        // ✅ 如果提供了初始日期，使用它；否则使用默认日期1997年1月1日
        let dateToUse: Date
        if let initialDate = initialDate {
            ...
            components.month = 1
            components.day = 1
            dateToUse = calendar.date(from: components) ?? Date()
        }
        datePicker.date = dateToUse
        print("✅ BirthdayDatePopup: 初始日期：\(dateToUse)")
    ...
    func layout(width: CGFloat, bottomSpacing: CGFloat) {
        ...
        backView.layoutNow()
        backView.bounds = CGRect(x: 0, y: 0, width: backView.width(), height: datePicker.maxY() + 88 + bottomSpacing)
    }
}
