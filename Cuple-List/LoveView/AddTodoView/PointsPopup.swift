...
class PointsPopup: NSObject {
    ...
    var onPointselected: ((String) -> Void)?
    // ⭐️ 新增属性：存储当前选中的按钮
    ...
    private func setupUI() {
        ...
        continueButton.reactive.controlEvents(.touchUpInside).observeValues {
            ...
            if let selectedPoints = self.selectedPoints {
                self.onPointselected?(selectedPoints) // 传递选中的分数
