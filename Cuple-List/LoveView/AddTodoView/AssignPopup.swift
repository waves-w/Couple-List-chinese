...
class AssignPopup: NSObject {
    ...
    /// ✅ Anni 用：点击 A 切换 A 的选中状态（可多选：A、B、或双方）
    ...
    private func updateAssignIndexAndUI() {
        ...
        continueButton.isEnabled = selectedAssignIndex != -1
        continueButton.alpha = selectedAssignIndex != -1 ? 1.0 : 0.5
    }
    ...
    func show(width: CGFloat, bottomSpacing: CGFloat, initialIndex: Int? = nil) {
        ...
        let index = (initialIndex == TaskAssignIndex.both.rawValue) ? TaskAssignIndex.partner.rawValue : (initialIndex ?? -1)
               if index >= 0 && index <= 1 {
